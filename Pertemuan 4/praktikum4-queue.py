...
#Queue dengan 2 pointer : front and rear (sama kaya head and tail)
class QueueLL:
    def __init__(self):
    ...
    def enqueue(self, data):
        ...
        # jiks queue kosong, front dan rear menunjuk ke node yang sama
        if self.is_empty():
            self.front = nodeBaru
            self.rear = nodeBaru
            return
    ...
    def dequeue(self):
        ...
        # 2) geser front ke node berikutnya (next)
...
# instantiasi objek clas QueueLL
q = QueueLL()
q.enqueue("A")
q.enqueue("B")
q.enqueue("C")
